...
var TOP_SPACE = 100;
var RIGHT_SPACE = 50;
var bar_chart_svg = svg.append("svg")
                       .attr("width", width)
                       .attr("height", height)
...
d3.csv("USmapnew.csv", function(error, data) {
...
var g = {
  basemap: map_svg.append("g").attr("id", "basemap"),
  Issue: map_svg.append("g").attr("id", "Issues"),
  //tooltip: svg.append("g").attr("id", "tooltip"),
  legend: map_svg.append("g").attr("id", "legend"),
};
...
var zoom = d3.zoom()
    ...
    .scaleExtent([1, 8])
    .on("zoom", zoomed);
map_svg.call(zoom); // delete this line to disable free zooming
...
function zoomed() {
    map_svg.style("stroke-width", 1.5 / d3.event.transform.k + "px");
    // g.attr("transform", "translate(" + d3.event.translate + ")scale(" + d3.event.scale + ")"); // not in d3 v4
    //map_svg.attr("transform", d3.event.transform); // updated for d3 v4
